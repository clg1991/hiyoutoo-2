...
class MessageController extends Controller {
    ...
    public function message_page(){
         ...
         $condition="select distinct writerid from vote_message where (substring_index(relationid,'to',1)=$userid and readed=0 and writerid!=$userid)
         or (substring_index(relationid,'to',-1)=$userid and readed=0 and writerid!=$userid)";
         
         $usernewmessage=$Model->query($condition);

     foreach ($usernewmessage as $key => $msid){ 
    
      $User=M("User");
    
     $usernewmessage[$key]['messageyouname']=$User->where('userid='.$msid['writerid'])->getField('username');
    } 
      $this->assign('newmessagelist',$usernewmessage);
       $this->display();
    }
	...
	/*  -----------------------------------------------------首次加载聊天框的函数 */
	public function message($youuserid){ 
		
        layout(false);
		$User = M("User");
		$youusername=$User->where("userid='$youuserid'")->getField('username');  /* 根据对方名字寻找对面ID */
		$Message = M("Message");
