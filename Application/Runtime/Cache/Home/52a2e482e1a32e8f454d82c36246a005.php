...
<div style="min-height:700px;margin-top:50px" id='youtoo-body-content'> <link rel="stylesheet" type="text/css" href="/AA/Application/Public//Css/center.css" />
<script type="text/javascript" src="/AA/Application/Public//Js/center.js"></script>

<div class="center-module">
<div class="row" style='width:96%;margin:0 auto;margin-top:60px;'>
...
<?php if($page_user_id==$userid) { echo "<a href='/index.php/center/infoedit' style='float:right'>编辑资料</a></br>";} $User=M('User'); echo "性别:".$useremail=$User->where("userid=$page_user_id")->getField('sex')."</br>"; echo "邮箱:".$useremail=$User->where("userid=$page_user_id")->getField('email')."</br>"; echo "qq:".$useremail=$User->where("userid=$page_user_id")->getField('qq')."</br>"; echo "微信:".$useremail=$User->where("userid=$page_user_id")->getField('weixin')."</br>"; ?>
</div>
</div>
...
<?php if(is_array($voterecord)): $i = 0; $__LIST__ = $voterecord;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vtr): $mod = ($i % 2 );++$i; if($time < ($vtr['time']+3200)): echo ($vtr["username"]); ?>刚刚参与了投票
<span class='layout-center-votename'>

<?php $Content=M('Content'); $optioncontent=$Content->where("voteno=$vtr[voteno]")->getField("votename"); echo $optioncontent; ?>
</span>
 选择了选项
<span class='layout-center-option'>
<?php $Content=M('Content'); $optioncontent=$Content->where("voteno=$vtr[voteno]")->getField("option".$vtr['optionno']); echo $optioncontent; ?>
</span>
<?php else: ?>
...
<?php echo ($vtr["username"]); ?>在<?php echo ($vtr["time"]); ?>时参与了投票
<span class='layout-center-votename'>
<?php $Content=M('Content'); $optioncontent=$Content->where("voteno=$vtr[voteno]")->getField("votename"); echo $optioncontent; ?>
</span>
 选择了选项
